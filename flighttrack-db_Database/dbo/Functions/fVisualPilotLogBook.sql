...
CREATE FUNCTION [fVisualPilotLogBook]
(	
	@pilotId int
)
RETURNS TABLE 
...
with rte as(
	...
	where isnull(f.IsJunk,0) =0 and f.PilotID = @pilotId
...
,prefinal as (
	select fl.*, gmami.GPSLocationID ,gmami.order_id, AirportCode=ac.Code ,AirportID = ac.ID,g.longitude,g.latitude
	from fl
